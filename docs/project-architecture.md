...
登录 → token 存入 Storage
  ...
  → 刷新也失败 → 清除凭证 → 提示 r2-cli auth login
```
...
```bash
r2-cli goods up address --provinces              # 列出省份
r2-cli goods up address --cities <省>             # 列出城市
r2-cli goods up address --areas <市> --province <省>  # 列出地区
...
- `AuthError` → 提示 `请先运行 r2-cli auth login`
...
```
用户输入: r2-cli goods list --status wait
    │
    ▼ Commander 解析
...
Command Action
    ...
    try {
      ...
      handleCommandError(error)
        │
        ├── AuthError  → console.error("请先运行 r2-cli auth login")
