...
3. `src/commands/goods/` — 商品管理命令组（shops / stocks / list / listing / up）
...
- `modules/xianyu.ts` — 业务 API，导出函数：`getUserShopList` / `getUserStockList` / `getSelectGoodsList` / `listingUpXianyu` / `getListingInfo`。内部共享一个 `ApiClientService` 实例。
...
**共享工具** (`src/utils/`)：
- `qrcode.ts` — `renderQRCode(content, filename)` — 共享二维码渲染（PNG + Unicode 半块字符 + `qrUrl` 本地 HTTP 链接 + base64）。`startQrServer` 启动临时 HTTP 服务，通过 SSE 推送 `QrPageStatus`。QR 页面品牌色 `#06d290`。
...
- `mms/seller/goods/select/list` → `getSelectGoodsList({ stockId })` — 获取选品商品。`SelectGoodsItem.stockGoodsId` 是上架参数 `--stock-goods-id`。
- `POST mms/goods/listing/up/xianyu` → `listingUpXianyu()` — 提交上架
- `GET mms/goods/listing/get` → `getListingInfo()` — 查询上架进度

`goods up` 命令提交后自动轮询 `getListingInfo`（每 3 秒，最多 60 秒），返回 `{ submit, listing }` 包含提交和最终状态。
...
- `src/commands/shared.ts`：
  ...
  - `agentAction(fn)` — Agent 子命令包装器，自动 catch 并格式化为 JSON 错误。**注意**：被包装的函数需要自己 `console.log` 输出结果，`agentAction` 不处理返回值。
