...
- `modules/goods.ts` — 业务 API，导出函数：`getUserShopList` / `getUserStockList` / `getSelectGoodsList` / `listingUpXianyu` / `getListingInfo` / `listingDownXianyu` / `listingUpdatePrice` / `updateGoodsInfo` / `getListingList` / `listingHangUpXianyu` / `getXyCategories` / `getXyProps` / `getXyPropValues` / `uploadXyImages`。内部共享一个 `ApiClientService` 实例。
...
**关键约束**：
- 定位商品用 `--stock-goods-id + --account`（**不要用 `--id`**，列表返回的 `id` 字段不是 edit API 的 `goodsListingId`）
...
- `src/commands/shared.ts`：
  ...
  - goods 命令（shops/stocks/list/listing/down/price/edit/up）统一使用 inline try/catch + `if (options.json)` 在 catch 中输出 JSON 错误。`up` 命令额外在交互模式前验证 agent 参数完整性
  - auth 命令 login/xianyu 的 `--json` 分支复用 `src/commands/auth/qr-flow.ts` 的 `runQRJsonFlow()` 通用流程
