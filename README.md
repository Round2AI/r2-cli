...
| 命令 | 说明 |
|------|------|
| `r2-cli goods edit --stock-goods-id <id> --account <shopId> --category-id <id> --channel-cat-id <id> --json` | 修改商品信息（必填：定位参数 + 类目） |
| `r2-cli goods edit --stock-goods-id <id> --account <shopId> --category-id <id> --channel-cat-id <id> --image-ids <ids> --item-attrs <json> --brand-name <name> --json` | 带图片和属性修改 |

**必填参数**：`--stock-goods-id` + `--account`（定位商品）、`--category-id` + `--channel-cat-id`（后端必填）
