...
覆盖商品管理、认证登录、店铺管理等核心业务域，提供 20+ 命令及 3 个 AI Agent [Skills](./skills/)。
...
| 商品管理 | 上架、下架、重新上架、改价、商品列表、选品列表 |
| 店铺管理 | 查看已授权店铺（闲鱼/抖音，跨平台汇总） |
| 仓库管理 | 查看所有仓库 |
...
| 命令 | 说明 |
|------|------|
| `r2-cli goods shops -p <xianyu\|douyin>` | 查看已授权店铺（`--json`） |
| `r2-cli goods list --status <status>` | 商品列表（`--status`、`--keyword`、`--page`、`--size`、`--json`） |
...
| 命令 | 说明 |
|------|------|
| `r2-cli shops` | 查看所有已授权店铺（跨平台） |
| `r2-cli shops --json` | JSON 输出（Agent 用） |
| `r2-cli stocks` | 查看所有仓库 |
| `r2-cli stocks --json` | JSON 输出（Agent 用） |
| `r2-cli goods select` | 选品商品列表（今日数据，`--json`） |
| `r2-cli goods select --stock-id <id> --stock-goods-id <id>` | 按条件筛选 |
...
| 命令 | 说明 |
|------|------|
| `r2-cli report` | 经营日报/周报 |
| `r2-cli pricing` | 收货价与售卖价建议 |
| `r2-cli inventory` | 库存管理 |
