...
查询类命令（`goods list`、`goods listing`）支持分页：

```bash
# 每页 50 条（最大）
r2-cli goods list --stock-id <id> --page 1 --size 50 --json
r2-cli goods listing --status up --page 2 --size 50 --json
```

### 状态过滤

```bash
r2-cli goods listing --status up     # 已上架
r2-cli goods listing --status sold   # 已售出
r2-cli goods listing --status down   # 已下架
```
