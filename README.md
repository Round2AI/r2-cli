...
覆盖商品管理、认证登录、经营分析、市场分析、价格分析、库存履约等核心业务域，提供 40+ 命令及 3 个 AI Agent [Skills](./skills/)。
...
> Agent 不要直接运行 `r2 auth login`（交互式命令），使用两步式流程：
...
# Agent 分步上架（6 步）
r2 goods up info <id>          # 获取商品详情
r2 goods up categories         # 获取分类
r2 goods up props <catId>      # 获取属性
r2 goods up submit ...         # 提交上架
```
...
| 命令 | 说明 |
|------|------|
| `r2 auth login` | 扫码登录（交互式，人类使用） |
| `r2 auth login qr` | 生成二维码 JSON（Agent 第1步） |
| `r2 auth login poll --token <>` | 轮询登录状态 JSON（Agent 第2步） |
...
| `r2 goods list --status <status>` | 商品列表（wait/on/sold/down） |
| `r2 goods up` | 交互式上架（7 步向导） |
...
| 命令 | 说明 |
|------|------|
| `r2 goods up info <id>` | 获取商品详情 + 预填值 |
| `r2 goods up address` | 设置/查看发货地址 |
| `r2 goods up categories` | 获取分类树 |
| `r2 goods up props <catId>` | 获取分类属性 |
| `r2 goods up submit --data @file.json ...` | 提交上架 |

### 开发中

| 命令 | 说明 |
|------|------|
| `r2 report generate` | 经营日报/周报 |
| `r2 pricing analyze` | 价格分析建议 |
| `r2 inventory risk` | 风险管理 |
